...
export default function MaliyetAnalizPage() {
    ...
    const fetchData = async () => {
        setIsLoading(true);
        try {
            // Wait for both the fetch and a minimum delay of 1.5s
            const [res] = await Promise.all([
                fetch(`${API_URL}/finans/costing/analytics?tenantId=demo-tenant`),
    ...
    if (isLoading) {
        return <LoadingScreen />; // Wait, I need to import it first. 
    }
